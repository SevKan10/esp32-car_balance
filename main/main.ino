...
// ĐỊNH NGHĨA CÁC BIẾN THỰC TẾ (không extern)
...
double dt;           
float output;       
float setPoint = 0.8, error, lastError = 0; 
...
void setup() 
{
  ...
  if (i2cData[0] != 0x68) { Serial.print(F("Error reading sensor"));while (1);}
  delay(100); 
...
void loop() 
{
    ...
    if ((roll < -90 && kalAngleX > 90) || (roll > 90 && kalAngleX < -90)) 
    {
        ...
        kalAngleX = roll;
        gyroXangle = roll;
    } else {kalAngleX = kalmanX.getAngle(roll, gyroXrate, dt);}
    ...
    if ((pitch < -90 && kalAngleY > 90) || (pitch > 90 && kalAngleY < -90)) 
    {
        ...
        kalAngleY = pitch;
        gyroYangle = pitch;
    } else {kalAngleY = kalmanY.getAngle(pitch, gyroYrate, dt);}
