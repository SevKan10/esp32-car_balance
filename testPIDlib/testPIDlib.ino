...
void setup() {
  // Đặt ngưỡng Setpoint (ví dụ muốn giữ cân bằng ở góc 0)
  setPoint = 0.0;

  // Bật PID
  myPID.SetMode(AUTOMATIC);
  myPID.SetOutputLimits(-255, 255);  // Cho phép output từ -255 đến +255
...
/*----------------------------- KALMAN FILTER -----------------------------*/
 /* Update all the values */
  ...
  // ---- Lấy dữ liệu từ MPU (sau Kalman filter) ----
  input = kalAngleY;   // Giá trị góc Y sau khi lọc Kalman

  // ---- Tính PID ----
  myPID.Compute();  // Thư viện sẽ tính Output dựa trên Input & Setpoint

  // ---- Dùng Output ----
  // Output có thể là giá trị PWM cho motor, servo, v.v.
